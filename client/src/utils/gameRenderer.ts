...
interface TrailLineDirection {
  hasTopTrail: boolean;
  hasBottomTrail: boolean;
  hasLeftTrail: boolean;
  hasRightTrail: boolean;
}
...
export class GameRenderer {
  ...
  private trailLineDirections: Map<string, TrailLineDirection> = new Map(); // Key: "row:col:ownerId" - stores fixed line direction
  private trailCreationTime: Map<string, number> = new Map(); // Key: "row:col:ownerId" - stores when trail cell was created
  ...
  private readonly TRAIL_SESSION_THRESHOLD = 1000; // milliseconds - trails created within this time are considered same session
  ...
  constructor(
    ...
    bg2Img.src = '/elements/IngameUI/background2.png';
    bg2Img.onload = () => {
      this.background2Image = bg2Img;
    };
    bg2Img.onerror = () => {
      console.warn('Failed to load background2.png');
    };
  }

  render(gameState: GameState, camera: Camera, arena: Arena) {
    // Clear canvas with a visible color
  ...
  private drawGridLines(camera: Camera, gameState: GameState, arena: Arena) {
    ...
    // Helper to check if a cell is a trail cell (check trails overlay)
    const isTrailCell = (r: number, c: number) => {
      if (r < 0 || c < 0) return false;
      const key = `${r}:${c}`;
      return gameState.trails?.[key] !== undefined;
    };
  ...
  private drawCellWithStyle(
    ...
    // Skip if this is a trail (trails are rendered separately as overlay)
    if (cell.isTrail) {
      return;
    }
  ...
  private drawTrailBorder(
    ...
    // Set up stroke style - brighter border for better visibility
    this.ctx.strokeStyle = '#ffffffcc'; // Increased opacity from 55 to cc (80% opacity)
  ...
  /**
   * Draw trails overlay - trails are rendered on top of cells
   */
  private drawTrailsOverlay(camera: Camera, gameState: GameState, arena: Arena) {
    ...
    for (const [key, trail] of Object.entries(gameState.trails)) {
      if (!trail) continue; // Skip null trails (removed)
      ...
      // Draw trail with semi-transparent overlay
      const trailKey = `${row}:${col}:${trail.ownerId}`;
      const trailAnimation = this.trailAnimations.get(trailKey);
      
      let fillColor = trail.color;
      if (trailAnimation) {
      ...
      // Draw trail border
      this.drawTrailBorderOverlay(trail, x, y, row, col, gameState);
    }
  }

  /**
   * Draw border for trail overlay
   */
  private drawTrailBorderOverlay(trail: any, x: number, y: number, row: number, col: number, gameState: GameState) {
    const hasTopTrail = this.hasTrailAt(row - 1, col, trail.ownerId, gameState);
    const hasBottomTrail = this.hasTrailAt(row + 1, col, trail.ownerId, gameState);
    const hasLeftTrail = this.hasTrailAt(row, col - 1, trail.ownerId, gameState);
    const hasRightTrail = this.hasTrailAt(row, col + 1, trail.ownerId, gameState);
  ...
  /**
   * Check if there's a trail at given position for given owner
   */
  private hasTrailAt(row: number, col: number, ownerId: string, gameState: GameState): boolean {
    if (row < 0 || col < 0) return false;
    const key = `${row}:${col}`;
    const trail = gameState.trails?.[key];
    return trail && trail.ownerId === ownerId;
  }

  /**
   * Draw dashed white line in the middle of trail cells
   * Lines are drawn continuously across multiple cells
   * Uses fixed direction stored when trail cell was first created
   */
  private drawTrailDashedLine(
    ...
    const trailKey = `${row}:${col}:${ownerId}`;
    
    // Get fixed direction if it exists, otherwise calculate and store it
    let direction: TrailLineDirection;
    if (this.trailLineDirections.has(trailKey)) {
      // Use stored fixed direction
      direction = this.trailLineDirections.get(trailKey)!;
    } else {
      // Calculate direction based on current adjacent cells and store it
      // Only consider adjacent trail cells that are part of the current trail being drawn
      // This prevents new trails from connecting to old trails
      const rows = cells.length;
      const cols = cells[0]?.length || 0;

      if (!this.previousGameState) {
        // No previous state, can't determine if it's part of current trail
        // Set all directions to false (endpoint)
        direction = {
          hasTopTrail: false,
          hasBottomTrail: false,
          hasLeftTrail: false,
          hasRightTrail: false
        };
        this.trailLineDirections.set(trailKey, direction);
      } else {
        const now = performance.now();
        const isCurrentTrailCell = (r: number, c: number) => {
          if (r < 0 || c < 0 || r >= rows || c >= cols) return false;
          const nRow = cells[r];
          if (!nRow) return false;
          const nCell = nRow[c];
          if (!nCell || !nCell.isTrail || nCell.ownerId !== ownerId) {
            return false;
          }
          
          // Check previous state
          const prevNRow = this.previousGameState.cells[r];
          const prevNCell = prevNRow ? prevNRow[c] : null;
          
          // Only consider it if it was a trail in previous state (part of current trail being drawn)
          const wasTrailInPrevious = prevNCell && prevNCell.isTrail && prevNCell.ownerId === ownerId;
          
          if (wasTrailInPrevious) {
            return true;
          }
          
          // Check if it's a trail cell created in the same session
          const adjacentTrailKey = `${r}:${c}:${ownerId}`;
          const adjacentCreationTime = this.trailCreationTime.get(adjacentTrailKey);
          const currentCreationTime = this.trailCreationTime.get(trailKey);
          
          if (adjacentCreationTime !== undefined && currentCreationTime !== undefined) {
            const timeDiff = Math.abs(currentCreationTime - adjacentCreationTime);
            return timeDiff < this.TRAIL_SESSION_THRESHOLD;
          }
          
          // If this cell has a stored direction, it's part of a trail session
          // Only connect if the adjacent cell also has a stored direction (same session)
          return this.trailLineDirections.has(adjacentTrailKey);
        };

        direction = {
          hasTopTrail: isCurrentTrailCell(row - 1, col),
          hasBottomTrail: isCurrentTrailCell(row + 1, col),
          hasLeftTrail: isCurrentTrailCell(row, col - 1),
          hasRightTrail: isCurrentTrailCell(row, col + 1)
        };
        
        // Store the direction so it remains fixed
        this.trailLineDirections.set(trailKey, direction);
      }
    }

    const hasTopTrail = direction.hasTopTrail;
    const hasBottomTrail = direction.hasBottomTrail;
    const hasLeftTrail = direction.hasLeftTrail;
    const hasRightTrail = direction.hasRightTrail;
    ...
    if (isCorner) {
      ...
      // Always draw L-shape for corners to ensure proper connection
      if (hasTopTrail && hasLeftTrail) {
        // Top-left corner: L-shape from top edge to center, then to left edge
        this.ctx.moveTo(centerX, y);
        this.ctx.lineTo(centerX, centerY);
        this.ctx.lineTo(x, centerY);
      } else if (hasTopTrail && hasRightTrail) {
        // Top-right corner: L-shape from top edge to center, then to right edge
        this.ctx.moveTo(centerX, y);
        this.ctx.lineTo(centerX, centerY);
        this.ctx.lineTo(x + this.cellSize, centerY);
      } else if (hasBottomTrail && hasLeftTrail) {
        // Bottom-left corner: L-shape from bottom edge to center, then to left edge
        this.ctx.moveTo(centerX, y + this.cellSize);
        this.ctx.lineTo(centerX, centerY);
        this.ctx.lineTo(x, centerY);
      } else if (hasBottomTrail && hasRightTrail) {
        // Bottom-right corner: L-shape from bottom edge to center, then to right edge
        this.ctx.moveTo(centerX, y + this.cellSize);
        this.ctx.lineTo(centerX, centerY);
        this.ctx.lineTo(x + this.cellSize, centerY);
      } else {
        // Fallback for other corner combinations - always draw L-shape
        // Determine which two directions to connect
        if (hasTopTrail) {
          this.ctx.moveTo(centerX, y);
          this.ctx.lineTo(centerX, centerY);
          if (hasLeftTrail) {
            this.ctx.lineTo(x, centerY);
          } else if (hasRightTrail) {
            this.ctx.lineTo(x + this.cellSize, centerY);
          } else if (hasBottomTrail) {
          ...
          this.ctx.moveTo(x + this.cellSize, centerY);
          this.ctx.lineTo(centerX, centerY);
        }
      }
  ...
  private deepCloneGameState(gameState: GameState): GameState {
    return {
      ...
      trails: gameState.trails ? { ...gameState.trails } : undefined,
   ...
   * Also stores fixed line direction for newly created trail cells
   */
  private detectNewTrailCells(gameState: GameState) {
    if (!this.previousGameState || !gameState.trails) {
      return;
    }
    ...
    for (const [key, trail] of Object.entries(gameState.trails)) {
      if (!trail) continue; // Skip null trails
      
      const [row, col] = key.split(':').map(Number);
      const prevTrail = this.previousGameState.trails?.[key];
      
      // Check if this is a newly created trail cell
      if (!prevTrail || prevTrail.ownerId !== trail.ownerId) {
        const trailKey = `${row}:${col}:${trail.ownerId}`;
        
        // Store creation time for this trail cell
        this.trailCreationTime.set(trailKey, now);
        
        // Store fixed line direction when trail cell is first created
        const isCurrentTrailCell = (r: number, c: number) => {
          if (r < 0 || c < 0) return false;
          const nKey = `${r}:${c}`;
          const nTrail = gameState.trails?.[nKey];
          if (!nTrail || nTrail.ownerId !== trail.ownerId) {
            return false;
          }
          
          // Check previous state
          const prevNTrail = this.previousGameState.trails?.[nKey];
          
          // Only consider it if:
          // 1. It was a trail in previous state (part of current trail being drawn), OR
          // 2. It's also a newly created trail in this same frame (created within threshold)
          const wasTrailInPrevious = prevNTrail && prevNTrail.ownerId === trail.ownerId;
          
          if (wasTrailInPrevious) {
            return true;
          }
          
          // Check if it's a new trail cell created in the same session
          const adjacentTrailKey = `${r}:${c}:${trail.ownerId}`;
          const adjacentCreationTime = this.trailCreationTime.get(adjacentTrailKey);
          if (adjacentCreationTime !== undefined) {
            const timeDiff = Math.abs(now - adjacentCreationTime);
            return timeDiff < this.TRAIL_SESSION_THRESHOLD;
          }
        ...
        const direction: TrailLineDirection = {
          hasTopTrail: isCurrentTrailCell(row - 1, col),
          hasBottomTrail: isCurrentTrailCell(row + 1, col),
          hasLeftTrail: isCurrentTrailCell(row, col - 1),
          hasRightTrail: isCurrentTrailCell(row, col + 1)
        ...
        if (!this.trailAnimations.has(trailKey)) {
          this.trailAnimations.set(trailKey, {
            startTime: now,
            cellKey: trailKey,
            targetColor: trail.color
          });
        }
      }
    }
    
    // Clean up directions for trail cells that no longer exist
    this.trailLineDirections.forEach((direction, key) => {
      const [rowStr, colStr, ownerId] = key.split(':');
      const row = parseInt(rowStr, 10);
      const col = parseInt(colStr, 10);
      
      const trailKey = `${row}:${col}`;
      const trail = gameState.trails?.[trailKey];
      if (!trail || trail.ownerId !== ownerId) {
        this.trailLineDirections.delete(key);
        this.trailCreationTime.delete(key);
      }
    });
  }
