...
class RoomManager {
    ...
    createRoom(isPrivate = false) {
        const roomId = uuidv4();
        // Both private and public rooms get a code
        ...
        console.log(`Created ${isPrivate ? 'private' : 'public'} room ${roomId} with code ${roomCode}`);
        return room;
    }
    ...
    findPublicRoom() {
        // Find a public room with space and not started (don't create new one)
        for (const roomId of this.publicRooms) {
            const room = this.rooms.get(roomId);
            if (room && room.players.size < this.MAX_PLAYERS && !room.gameStarted) {
                return room;
    ...
    getAllRooms() {
        ...
        for (const [roomId, room] of this.rooms.entries()) {
            // Only return rooms that haven't started
            if (!room.gameStarted) {
                roomsList.push({
                    id: room.id,
                    code: room.code,
